...
class Batch(object):
    ...
    def __init__(self, args, data=None, device=None, is_test=False, idf_info=None):
        ...
        # print("*********idf_info*********") # {'all': Counter(), 'customer': Counter(), 'agent': Counter(), 'num': 0, 'voc_size': 0}?
        # print(idf_info)
        # assert 0
        if data is not None:
            self.args = args
            ...
            # mask_src = 1 - (src == 0) # linux
            # mask_tgt = 1 - (tgt == 0) # linux
            mask_src = ~ (src == 0) # windows
            mask_tgt = ~ (tgt == 0) # windows
            ...
            if args.topic_model:
                all_bow, customer_bow, agent_bow = self.generate_bow(data, idf_info) # 三种角色的BOW，all_bow不为空，其余两个为空
                ...
                summ_all_bow, summ_cust_bow, summ_agent_bow = self.generate_summ_bow(data, idf_info) # 作用是剔除整体、优点、缺点三个部分BOW中的停用词或无效词（是根据所有all_file_count保留5<词频<6000的词，这个怎样统计去修改）
    ...
    def generate_summ_bow(self, data, idf_info):
        ...
        for idx in range(self.batch_size):
            ...
            for key in customer_counter.keys():
                ...
                customer_file_count = all_file_counter[key]
                if not self.args.use_idf:
            ...
            for key in agent_counter.keys():
                ...
                agent_file_count = all_file_counter[key]
                if not self.args.use_idf:
    ...
    def generate_bow(self, data, idf_info):
        ...
        for idx in range(self.batch_size):
            ...
            # print(all_counter) # Counter({0: 269, 1: 151, 3: 128, 4: 119, 5: 116, 2: 102, 6: 98, 11: 77,...})
            # print(customer_counter) # Counter()
            # print(agent_counter) # Counter()
            # assert 0
            all_counter_sum = sum(all_counter.values())
            # print(all_counter_sum) # 3171
            # assert 0
...
class DataIterator(object):
    ...
    def preprocess(self, ex, is_test):
        ...
        for turn in session:
            ...
            original_txt = "(" + str(index) + ') 【评论】 ' + original_txt
