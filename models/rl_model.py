...
from models.topic import MultiTopicModel01
...
class Model(nn.Module):
    def __init__(self, args, device, vocab, checkpoint=None):
        ...
        if args.encoder == 'bert':
            ...
            self.encoder = Bert('bert-base-cased', args.finetune_bert)
            if(args.max_pos > 512):
        ...
        if self.args.topic_model:
            ...
            self.topic_model = MultiTopicModel01(voc_emb.size(0), voc_emb.size(-1), args.topic_num, args.noise_rate,
                                               voc_emb, agent=args.agent, cust=args.cust)
    ...
    def pretrain(self, batch):
        ...
        if self.args.topic_model:
            ...
            all_bow, customer_bow, agent_bow = \
                batch.all_bow, batch.all_bow, batch.all_bow
            if self.args.split_noise:
                summ_all, summ_customer, summ_agent = \
                    batch.summ_all, batch.summ_customer, batch.summ_agent
                # summ_all, summ_customer, summ_agent = \
                #     batch.summ_all, batch.summ_all, batch.summ_all
            else:
