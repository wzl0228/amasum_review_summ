...
if __name__ == '__main__':
    ...
    # parser.add_argument("-bert_dir", default='bert/chinese_bert') # 中文bert
    ...
    parser.add_argument("-dec_layers", default=3, type=int) # 解码器是3层Transformer decoder
    ...
    parser.add_argument("-topic_model", type=str2bool, nargs='?', default=False) # 默认使用主题模型
    # parser.add_argument("-topic_model", type=str2bool, nargs='?', const=False, default=False) # 默认不使用主题模型
    ...
    parser.add_argument("-topic_num", default=50, type=int) # 主题数量
    # parser.add_argument("-topic_num", default=100, type=int) # 主题数量增加到100 
    ...
    parser.add_argument("-max_word_count", default=6000, type=int) # 所有文件统计某个词数量超过6000词，主题模型会认为是停用词
    parser.add_argument("-min_word_count", default=5, type=int) # 所有文件统计某个词数量小于5次，主题模型也不处理该词
    # parser.add_argument("-min_word_count", default=1, type=int) # 所有文件统计某个词数量小于1次，主题模型也不处理该词
    parser.add_argument("-agent", type=str2bool, nargs='?', default=True)
    parser.add_argument("-cust", type=str2bool, nargs='?', default=True)
    ...
    parser.add_argument("-max_tgt_len", default=303, type=int) # 训练时选取的最大摘要长度，这个参数修改100->303

    # Beam search decoding args
    parser.add_argument("-alpha",  default=0.95, type=float) # 0.6
    parser.add_argument("-beam_size", default=3, type=int)
    parser.add_argument("-min_length", default=10, type=int)
    parser.add_argument("-max_length", default=303, type=int) # 生成摘要的最大长度100，这个参数修改100->303
    ...
    # parser.add_argument("-lr_other", default=1e-4, type=float)
    # parser.add_argument("-lr_topic", default=1e-6, type=float)
    ...
    parser.add_argument("-pretrain_steps", default=80000, type=int) # 预训练80000个step
    ...
    parser.add_argument("-ex_max_token_num", default=500, type=int) # 提取出句子的最大长度
    ...
    parser.add_argument("-pn_layers", default=2, type=int) # 两层Transformer decoder
    ...
    if torch.cuda.is_available():
        ...
        args.visible_gpus = str(torch.cuda.current_device())
        # print(args.visible_gpus) # 0
